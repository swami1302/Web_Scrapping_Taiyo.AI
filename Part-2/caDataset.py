...
# Define the URL
url = 'https://data.ca.gov/dataset/?q=construction+and+infrastructure+projects&sort=score+desc%2C+metadata_modified+desc'
...
try:
    ...
    first_elements = soup.find_all(class_='dataset-heading')
    second_elements = soup.find_all(class_='dataset-content')
    ...
    with open('./Excel/caDataset.csv', 'w', newline='', encoding='utf-8') as csvfile:
        # Create a CSV writer object
        writer = csv.writer(csvfile)

        # Write headers to the CSV file
        writer.writerow(['Content from 1', 
                         'Content from 2'])  # Replace 'YourClassHere' with the third class name

        # Write the content to the CSV file
        max_rows = max(len(first_content_list), len(second_content_list))
        for i in range(max_rows):
            ...
            writer.writerow([first_content, second_content])
